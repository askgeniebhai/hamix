...
const ALLOWED_ORIGIN = process.env.HAMIX_ALLOWED_ORIGIN || '*';
...
const logEvent = (level, event, metadata = {}) => console.log(JSON.stringify({ level, event, timestamp: now(), ...metadata }));
const safeError = (res, status, message, code = 'request_error') => json(res, status, { error: message, code });
...
const setSessionCookie = (res, token, maxAge) => {
  const parts = [`hamix_session=${encodeURIComponent(token)}`, 'HttpOnly', 'Path=/', 'SameSite=Lax', 'Priority=High', `Max-Age=${maxAge}`];
...
const readBody = async (req) => new Promise((resolve, reject) => {
  ...
  req.on('end', () => {
    ...
    catch { reject(Object.assign(new Error('Malformed JSON request body.'), { statusCode: 400 })); }
  });
  req.on('error', reject);
});
...
const hasRole = (session, role) => (ROLE_RANK[session?.role] || 0) >= (ROLE_RANK[role] || 0);
const requireRole = (session, res, role) => {
  if (!hasRole(session, role)) {
    safeError(res, 403, `${role} role required.`, 'permission_denied');
    return false;
  }
  return true;
};
const rateLimit = (req, res, { limit = 120, windowMs = 60_000 } = {}) => {
  const key = `${req.socket.remoteAddress || 'local'}:${new URL(req.url, 'http://localhost').pathname}`;
  const current = requestAttempts.get(key) || { count: 0, resetAt: Date.now() + windowMs };
  if (current.resetAt < Date.now()) { current.count = 0; current.resetAt = Date.now() + windowMs; }
  current.count += 1;
  requestAttempts.set(key, current);
  if (current.count > limit) {
    safeError(res, 429, 'Too many requests. Try again later.', 'rate_limited');
    return false;
  }
  return true;
};
const providerStatus = () => ({
  ai: { configured: Boolean(process.env.HAMIX_AI_PROVIDER && process.env.HAMIX_AI_API_KEY), required: ['HAMIX_AI_PROVIDER', 'HAMIX_AI_API_KEY'] },
  deployment: { configured: Boolean(process.env.HAMIX_DEPLOYMENT_PROVIDER && process.env.HAMIX_DEPLOYMENT_TARGET), required: ['HAMIX_DEPLOYMENT_PROVIDER', 'HAMIX_DEPLOYMENT_TARGET'] },
  storage: { configured: Boolean(process.env.HAMIX_STORAGE_PROVIDER && process.env.HAMIX_STORAGE_BUCKET), required: ['HAMIX_STORAGE_PROVIDER', 'HAMIX_STORAGE_BUCKET'] },
  email: { configured: Boolean(process.env.HAMIX_EMAIL_PROVIDER && process.env.HAMIX_EMAIL_API_KEY), required: ['HAMIX_EMAIL_PROVIDER', 'HAMIX_EMAIL_API_KEY'] },
  sms: { configured: Boolean(process.env.HAMIX_SMS_PROVIDER && process.env.HAMIX_SMS_API_KEY), required: ['HAMIX_SMS_PROVIDER', 'HAMIX_SMS_API_KEY'] },
  monitoring: { configured: Boolean(process.env.HAMIX_MONITORING_PROVIDER), required: ['HAMIX_MONITORING_PROVIDER'] },
  analytics: { configured: Boolean(process.env.HAMIX_ANALYTICS_PROVIDER), required: ['HAMIX_ANALYTICS_PROVIDER'] },
  secretStorage: { configured: Boolean(process.env.HAMIX_SECRET_PROVIDER), required: ['HAMIX_SECRET_PROVIDER'] }
});
const readiness = () => {
  const issues = [];
  if (SESSION_SECRET === 'development-secret-change-me') issues.push('HAMIX_SESSION_SECRET must be changed for production.');
  if (NODE_ENV === 'production' && !COOKIE_SECURE) issues.push('HAMIX_COOKIE_SECURE=true is required in production HTTPS deployments.');
  try { run('PRAGMA quick_check;'); } catch { issues.push('SQLite quick_check failed.'); }
  return { ok: issues.length === 0, issues, providers: providerStatus() };
};
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (!rateLimit(req, res, { limit: url.pathname.startsWith('/api/auth/login') ? 20 : 240 })) return;
    if (req.method === 'GET' && url.pathname === '/api/health') return json(res, 200, { ok: true, uptime: process.uptime(), timestamp: now() });
    if (req.method === 'GET' && url.pathname === '/api/ready') return json(res, 200, readiness());
    ...
    if (req.method === 'GET' && url.pathname === '/api/providers/status') return json(res, 200, { data: providerStatus() });
    if (req.method === 'GET' && url.pathname === '/api/workspace/members') {
      if (!requireRole(session, res, 'Admin')) return;
      const members = all(`SELECT users.id, users.email, users.name, workspace_memberships.role, workspace_memberships.created_at FROM workspace_memberships JOIN users ON users.id = workspace_memberships.user_id WHERE workspace_memberships.workspace_id=${sqlString(session.workspace_id)} ORDER BY workspace_memberships.created_at ASC`);
      return json(res, 200, { data: members });
    }
    if (req.method === 'POST' && url.pathname === '/api/workspace/members') {
      if (!requireRole(session, res, 'Owner')) return;
      const body = await readBody(req);
      const role = body.role || 'Member';
      if (!['Admin', 'Member'].includes(role)) return safeError(res, 400, 'Owner can create Admin or Member users only.', 'invalid_role');
      const email = String(body.email || '').trim().toLowerCase();
      if (!email || !body.name || !body.password || String(body.password).length < 8) return safeError(res, 400, 'Name, email, role, and 8+ character password are required.', 'validation_error');
      let user = one(`SELECT * FROM users WHERE email=${sqlString(email)}`);
      if (!user) {
        const userId = id('user');
        const salt = crypto.randomBytes(16).toString('hex');
        run(`INSERT INTO users VALUES (${sqlString(userId)}, ${sqlString(email)}, ${sqlString(body.name)}, ${sqlString(hashPassword(body.password, salt))}, ${sqlString(salt)}, ${sqlString(now())})`);
        user = one(`SELECT * FROM users WHERE id=${sqlString(userId)}`);
      }
      const existing = one(`SELECT user_id FROM workspace_memberships WHERE user_id=${sqlString(user.id)} AND workspace_id=${sqlString(session.workspace_id)}`);
      if (existing) return safeError(res, 409, 'User is already a member of this workspace.', 'duplicate_member');
      run(`INSERT INTO workspace_memberships VALUES (${sqlString(user.id)}, ${sqlString(session.workspace_id)}, ${sqlString(role)}, ${sqlString(now())})`);
      audit(session, 'workspace_member_created', 'user', user.id, { role, email });
      return json(res, 201, { id: user.id, email: user.email, name: user.name, role });
    }
    if (req.method !== 'GET' && !requireRole(session, res, 'Admin')) return;
    ...
    if (printMatch && req.method === 'GET') {
      ...
      const html = `<!doctype html><html><head><title>${escapeHtml(proposal.proposalNumber)}</title><style>body{font-family:Inter,Arial,sans-serif;margin:40px;color:#0f172a}table{width:100%;border-collapse:collapse}td,th{border:1px solid #e5e7eb;padding:8px;text-align:left}.total{text-align:right;font-weight:700}.print-note{color:#475569}</style></head><body><h1>HAMIX Proposal</h1><h2>${escapeHtml(proposal.proposalNumber)} · v${escapeHtml(proposal.version)}</h2><p>Status: ${escapeHtml(proposal.status)}</p><h3>${escapeHtml(proposal.title)}</h3><p>${escapeHtml(proposal.scope)}</p><h4>Deliverables</h4><ul>${deliverables.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul><table><thead><tr><th>Description</th><th>Qty</th><th>Unit</th><th>Tax</th><th>Discount</th><th>Total</th></tr></thead><tbody>${lineItems.map(item => `<tr><td>${escapeHtml(item.description)}</td><td>${escapeHtml(item.quantity)}</td><td>${escapeHtml(item.unitPrice)}</td><td>${escapeHtml(item.tax)}</td><td>${escapeHtml(item.discount)}</td><td>${escapeHtml(item.lineTotal)}</td></tr>`).join('')}</tbody></table><p class="total">Total ${escapeHtml(proposal.currency)} ${escapeHtml(proposal.total)}</p><p>Validity: ${escapeHtml(proposal.validityDate)}</p><p>Terms: ${escapeHtml(proposal.paymentTerms)}</p><p class="print-note">Use your browser print dialog to save or print this reviewed proposal. This view does not include e-signature or email delivery.</p></body></html>`;
      res.writeHead(200, { ...securityHeaders(), 'content-type': 'text/html; charset=utf-8', 'content-security-policy': "default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; frame-ancestors 'none'" });
      return res.end(html);
    }
    ...
    logEvent(status >= 500 ? 'error' : 'warn', 'api_request_failed', { method: req.method, path: req.url, status, message: error.message });
    return safeError(res, status, status >= 500 ? 'Request failed.' : error.message, status >= 500 ? 'internal_error' : 'request_error');
  }
});
